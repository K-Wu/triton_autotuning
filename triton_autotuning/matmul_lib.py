...
def _matmul_kernel(
    ...
    for ki in range(
        ...
        if even_k:
            ...
            a = tl.load(
                _fix_type_for_load(lhs), mask=rk[None, :] < ki, other=0
            )
            b = tl.load(
...
def to_triton_f8(x: torch.Tensor) -> triton.TensorWrapper:
    ...
    ret = triton.reinterpret(
        torch.empty(
            x.shape, dtype=torch.int8, device=x.device, layout=x.layout
        ),
        tl.float8e5,
    )
...
def benchmark_matmul_tiling(
    ...
    with torch.cuda.stream(s):
        try:
            ...
            LOG.error(
                "%s for %s generated %s", tiling, dims, exc, exc_info=True
            )
            raise
...
def _init_matmul_argument(
    ...
    if quantization == QuantizedInputType.INT8:
        return torch.randint(
            0, 128, (rows, cols), device="cuda", dtype=torch.int8
        )
...
def print_roofline_performance(dims: MatmulSize, time_ms: float):
    ...
    bound = (
        "bandwidth"
        if roofline_time_ms_bw > roofline_time_ms_flops
        else "flops"
    )
